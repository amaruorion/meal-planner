class MealPlanner {
    constructor() {
        this.meals = this.loadFromStorage('meals') || [];
        ...
        this.weeklyPlan = this.loadFromStorage('weeklyPlan') || {};
        
        this.archive = this.loadFromStorage('archive') || [];
        this.personNames = this.loadFromStorage('personNames') || {
            person1: 'Person 1',
            person2: 'Person 2', 
            person3: 'Person 3',
            person4: 'Person 4',
    ...
    initializeStaticEventListeners() {
        ...
        document.getElementById('export-archive-btn').addEventListener('click', () => this.exportArchive());
    ...
    exportArchive() {
        if (this.archive.length === 0) {
            alert('No archived weeks to export.');
            return;
        }
        ...
        link.href = url;
        link.download = `meal-planner-archive-${new Date().toISOString().split('T')[0]}.json`;
        link.click();
